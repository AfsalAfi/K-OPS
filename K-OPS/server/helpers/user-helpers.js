  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      for (var i = 0; i < doctors.length; i++) {
        ...
        var time = moment(doctors[i].availableTimeTo, "hh:mm A").format(
          "HH:mm"
        );
        var availableTimeToFloat = parseFloat(time.replace(":", "."));

        var time = moment(doctors[i].availableTimeFrom, "hh:mm A").format(
          "HH:mm"
        );
        var availableTimeFromFloat = parseFloat(time.replace(":", "."));

        console.log(
          availableTimeFromFloat,
          currentTimeFloat,
          availableTimeToFloat
        );

        if (
          doctors[i].availableTimeFrom.split(" ")[1] === "PM" &&
          (currentTime.split(" ")[1] === "AM" ||
            currentTime.split(" ")[1] === "PM") &&
          doctors[i].availableTimeTo.split(" ")[1] === "AM"
        ) {
          console.log("keri");
          if (
            doctors[i].availableTimeFrom.split(" ")[1] === "PM" &&
            ...
            if (
              availableTimeFromFloat < currentTimeFloat &&
              currentTimeFloat > availableTimeToFloat
            ) {
        ...
        collection
          ...
          .then((response) => {
            if (response.acknowledged === true) {
              ...
              return reject({
                message: "Process failed try again after some time...",
              });
            }
          });
      } catch (error) {
  ...
  list_Medical_Facilities: (hospital) => {
    return new Promise(async (resolve, reject) => {
      try {
        const collection = await getCollection(OPERATORS_COLLECTION);
        collection
          .find({ regId: hospital, classify: "Hospital" })
          .toArray()
          .then((response) => {
            resolve({
              facilities: response[0].facilities[0],
              equipments: response[0].equipments[0],
            });
          });
      } catch (err) {
